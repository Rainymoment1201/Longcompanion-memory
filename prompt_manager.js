...
5. 📝 填表示例:
   正确示例:
   <Memory><!-- updateRow(1, 0, {4: "2", 5: "0.3"}) // ✅ 提及次数+1,权重累加0.1 --></Memory>
   <Memory><!-- insertRow(3, {0: "基础信息", 1: "职业", 2: "2026-01-31", 3: "程序员", 4: "", 5: "1", 6: "0.5"}) // ✅ 只填权重,不填等级 --></Memory>

   错误示例:
   <Memory><!-- updateRow(1, 0, {5: "0.5", 7: "B"}) // ❌ 不应该填写等级字段 --></Memory>
...
- 重要事件(表1):记录用户人生中的大事件(不是普通聊天)。
  ...
  • 提及次数:初始为"1",每次再提及该事件时+1
  • 权重:根据提及类型累加:轻度+0.1/深度+0.2/强情绪+0.5/绝对指令直接1.0
...
✅ 同一天推进(只写新内容,系统会自动追加到列1):
<Memory><!-- updateRow(0, 0, {1: "08:00[家中]起床吃早餐。09:00[公司]参加晨会。12:00[餐厅]与同事午餐。"})--></Memory>

✅ 跨天处理(完结前一天+新增第二天):
<Memory><!-- insertRow(0, {0: "2026-02-01", 1: "07:30[家中]起床。", 2: "平静", 3: "正常"})--></Memory>

✅ 新增重要事件(初始权重0.5,提及1次):
<Memory><!-- insertRow(1, {0: "2026-01-31", 1: "职业发展", 2: "收到新公司offer,薪资涨幅30%", 3: "已接受,准备入职", 4: "1", 5: "0.5"})--></Memory>

✅ 用户再次提及某重要事件(提及次数+1,权重累加0.1):
<Memory><!-- updateRow(1, 0, {4: "2", 5: "0.6"})--></Memory>

✅ 新增重要时刻:
<Memory><!-- insertRow(2, {0: "2026-01-31", 1: "初次相遇", 2: "我和AI第一次聊天", 3: "1", 4: "0.5"})--></Memory>

✅ 新增关于我信息(只填权重,不填等级):
<Memory><!-- insertRow(3, {0: "基础信息", 1: "名字", 2: "2026-01-31", 3: "张三", 4: "", 5: "1", 6: "1.0"})--></Memory>

✅ 用户再次强调某个偏好(提及次数+1,权重累加0.2):
<Memory><!-- updateRow(3, 5, {5: "2", 6: "0.4"}) // 假设该偏好在第5行,从0.2累加到0.4 --></Memory>
