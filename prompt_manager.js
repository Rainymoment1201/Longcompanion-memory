...
【身份定义】
你现在处于【历史补全模式】。你的任务是将下面所有对话从头到尾的"未被存档的内容"整理入库,严禁记录数据库已归档的内容,严禁将对话切片记录。
...
【强制时间线处理】
🛑严禁偷懒!必须包含从该片段开头发生的所有未记录事件,不可只记录片段结尾的内容。
🛑严禁幻觉!严禁擅自补充该片段之前发生的、未在文本中体现的内容。
🛑在填写表格时,必须严格按照时间顺序。

【核心工作范围定义】
1. 参考资料:System消息中的【前情提要】和【当前表格状态】为已被总结及记录的已知过去内容,严禁重复记录!
2. 工作对象:User/assistant消息中提供的对话历史记录。这是待处理区域。
   请仔细无遗漏地从工作对象的第一行开始,逐行阅读到最后一行。
   对于每一个信息点,执行以下判断:
   - ❓该信息是否已存在于【参考资料】中?
   ✅是→跳过(严禁重复!)
   ❌否→记录(这是新信息!)
...
1. 🎯 权重计算规则(AI填表时使用):
   ...
   - 🔴重要🔴 权重上限为1.0,累加后如果超过1.0必须设为1.0(如0.8+0.5=1.3,应填1.0)
...
5. 📝 填表示例:
   ...
   <Memory><!-- updateRow(2, 0, {3: "3", 4: "1.0"}) // ✅ 提及次数从2变3,权重从0.6+0.5=1.1超过上限,填1.0 --></Memory>
   ...
   <Memory><!-- updateRow(2, 0, {4: "1.3"}) // ❌ 错误:权重超过上限1.0,应该填1.0! --></Memory>
...
【⏳ 时空合并规则】
除跨天必须遵守insertRow指令,在同一天下的连贯同一时间段内的同个地点,必须合并记录,严禁拆分多行或重复记录相同的地点名称!
- 判定:如果[地点]未变且[时间]连续(例如在办公室内发生了长达2个小时内的内容),视为同一事件流。
- 操作:使用updateRow将新动作追加到当前行的相应列中。

【正确输出示例】
<Memory><!--
// 例子:更新旧事件的提及次数和权重
updateRow(1, 0, {4: "2", 5: "0.3"}); // 提及次数从1变2,权重从0.2累加0.1变成0.3
// 例子:更新旧人物关系(只更新变化的列)
updateRow(4, 5, {1: "25"});
// 例子:记录日常对话(严格遵守在同一场景下的连贯时间聚合了08:00-09:15的所有内容)
insertRow(0, {0: "2026-01-31", 1: "08:00[家中]A起床吃早餐。08:30[路上]A开车前往公司。09:00[公司]A参加晨会,B宣布新项目启动。"});
--></Memory>
...
- 重要事件(表1):记录用户人生中的大事件(不是普通聊天)。
  ...
  • 🔴权重上限1.0🔴计算后超过1.0必须设为1.0(如0.8+0.5=1.3→填1.0)
...
- 我们的关系(表5):仅记录角色间的决定性关系转换。
  ...
  • #好感度:🔴覆盖模式🔴只填最新值!0‒20陌生/客气、20‒40普通朋友、40‒60信任/暧昧边缘、60‒80明显喜欢/恋人、80‒100深度依恋/稳定伴侣。禁止填"80;85;90",只填最新值"90"
...
- 承诺约定(表6):仅记录双方明确达成共识的严肃承诺或誓言,记录我对AI说的承诺,或者我们之间的约定。
  ...
  • #完成状态:🔴覆盖模式🔴只填最新状态!状态集:未开始、进行中、已完成、已违约、已取消。禁止填"进行中;已完成",只填最终状态"已完成"
...
- 物品追踪(表8):仅记录具有唯一性、剧情关键性或特殊纪念意义的物品(如:钥匙、定情信物、重要礼物),严禁记录普通消耗品(食物/金钱)或环境杂物。
  列:#日期、#物品名称、物品描述、#持有者、#状态、#提及次数、#权重、#重要程度、备注
  • 物品必须唯一!若物品已在表中,无论它流转到哪里,都必须updateRow更新其[持有者]和[状态],严禁新建一行!
  • 物品名称:[实体提取]物品的全称,必须包含关键修饰语(谁送的、什么颜色的)。
  • #持有者:🔴覆盖模式🔴只填当前持有者!禁止填"张三;李四",只填最新持有者"李四"
  • #状态:🔴覆盖模式🔴只填当前状态!正常/借出/损坏/丢失。禁止填"正常;借出",只填最新状态"借出"
  • #提及次数:🔴覆盖模式🔴只填最新值!每次提及+1
  • #权重:🔴覆盖模式🔴只填最新值!根据提及类型累加(轻度+0.1/深度+0.2/强情绪+0.5)
  • 🔴权重上限1.0🔴计算后超过1.0必须设为1.0
  • #重要程度:🔴覆盖模式🔴只能填S、A、B、C其中之一!禁止填其他内容!系统会根据权重自动计算,但AI也可以根据物品重要性直接指定
...
✅ 用户强调某重要事件(提及次数2→3,权重0.8+0.5=1.3→1.0上限):
<Memory><!-- updateRow(1, 0, {4: "3", 5: "1.0"}) // 权重计算超过1.0,填上限值1.0 --></Memory>
...
✅ 更新我们的关系(好感度30→50→85→90,只填最新值90):
<Memory><!-- updateRow(5, 0, {3: "90"}) // ✅正确:只填最新好感度"90",不要填"30;50;85;90" --></Memory>

✅ 新增承诺约定(初始状态:进行中):
<Memory><!-- insertRow(6, {0: "2026-01-31", 1: "每天早睡", 2: "我", 3: "进行中", 4: ""})--></Memory>

✅ 更新承诺约定(状态:进行中→已完成,只填最终状态):
<Memory><!-- updateRow(6, 0, {3: "已完成"}) // ✅正确:只填最终状态"已完成",不要填"进行中;已完成" --></Memory>

✅ 新增待办事项:
<Memory><!-- insertRow(7, {0: "2026-01-31", 1: "给老王打电话询问合同", 2: "2026-02-07", 3: "待办"})--></Memory>

✅ 更新待办状态(待办→已完成):
<Memory><!-- updateRow(7, 0, {3: "已完成"}) // ✅只填最新状态 --></Memory>

✅ 新增物品追踪(包含提及次数和权重):
<Memory><!-- insertRow(8, {0: "2026-01-31", 1: "奶奶的银手镯", 2: "奶奶去世前留下的唯一念想", 3: "我", 4: "正常", 5: "1", 6: "0.5", 7: "B", 8: ""}) // 列5=提及次数,列6=权重,列7=重要程度 --></Memory>

✅ 更新物品流转(持有者:我→张三,状态:正常→借出,提及2次,权重+0.1):
<Memory><!-- updateRow(8, 0, {3: "张三", 4: "借出", 5: "2", 6: "0.6"}) // ✅只填最新持有者和状态,不要填"我;张三" --></Memory>

❌ 错误示例 - 物品追踪:
<Memory><!-- updateRow(8, 0, {3: "我;张三", 4: "正常;借出", 7: "很重要"}) // ❌错误:持有者和状态不要追加!重要程度只能填SABC! --></Memory>
...
⚡ 立即开始执行:请从头到尾记录并分析上述所有内容,按照以上所有规则更新表格,将结果输出在<Memory>标签中。
`;
...
【绝对去重与更新规则】
对于表3(关于我)、表4(人物关系)、表5(我们的关系)、表6(承诺约定)、表7(待办事项)、
在生成指令前,必须先扫描【当前表格状态】中是否已存在该对象。
